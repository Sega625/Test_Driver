...
//  Win64bit := IsWow64();                                                                         //
  ...
  else // Для Win 95, Win 98 ...                                                                 //
  begin                                                                                          //
    SetLength(LPTPort, 1);                                                                       //
    LPTPort[0].Name := 'LPT1';                                                                   //
    LPTPort[0].BaseAddr1 := $0378;                                                               //
                                                                                                 //
    DrvID :=    'WinRing0.vxd';                                                                  //
    DrvfName := 'WinRing0.vxd';                                                                  //
  end;                                                                                           //
                                                                                                 //
//  DrvID := 'AsrDrv101';                                                                          //
...
begin                                                                                            //
  ...
  begin                                                                                          //
    ...
    gHandle := INVALID_HANDLE_VALUE;                                                             //
                                                                                                 ...
                                                                                                 //
  if IsNT then ManageDriver(PChar(DrvID), PChar(DrvPath), OLS_DRIVER_REMOVE);                    //
                                                                                                 //
  inherited;                                                                                     //
end;                                                                                             //
///////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            begin                                                                                                             //
              ...
              begin                                                                                                           //
                ...
                begin                              // Проверим,                                                               //
                  Present := False;                // что                                                                     //
                                                   // устройство                                                              //
                  Str := UpperCase(tSL2[j]);       // присутствует                                                            //
                  if Pos('CONTROL', Str) <> 0 then //                                                                         //
                  begin                                                                                                       //
                    SetLength(LPTPort, Length(LPTPort)+1);                                                                    //
                                                                                                                              //
                    Reg.OpenKeyReadOnly(tKey+tSL1[i]+'\'+tSL2[j]);                                                            //
                    try
...
//                      LPTPort[Length(LPTPort)-1].BusN  := byte(ReadDWORD(@Data[8])); // Найдем в разделе PCI дальше             //
                      ...
                      SetLength(LPTPort, Length(LPTPort)-1); // Удалим устр-во без AllocConfig

                  end;                                                                                                        //
                end;                                                                                                          //
                if not Present then Continue; // Пропустим, если устройства нет                                               //
                                                                                                                              //
                for j := 0 to tSL2.Count-1 do                                                                                 //
                begin                                                                                                         //
                  ...
                  begin                                                                                                       //
                    Str := Reg.ReadString(tKey+tSL1[i]+'\'+tSL2[j], 'PortName', 'x');                                         //
                                                                                                                              //
                    if Pos('LPT', UpperCase(Str)) <> 0 then                                                                   //
                    begin                                                                                                     //
                      ...
                      SetLength(LPTPort, Length(LPTPort)-1); // Удалим, если не LPT                                           //
                  end;                                                                                                        //
                end;                                                                                                          //
              end;                                                                                                            //
            end;                                                                                                              //
...
//////////////////////////////////////////////////////////////////////
function TLPTDriver.IsNT(): Boolean;                                //
var                                                                 //
  osvi: OSVERSIONINFO;                                              //
begin                                                               //
  Result := False;                                                  //
  ...
  osvi.dwOSVersionInfoSize := sizeof(OSVERSIONINFO);                //
	GetVersionEx(osvi);                                               //
  if osvi.dwPlatformId = VER_PLATFORM_WIN32_NT then Result := True; //
                                                                    //
//  ErrMess(IntToStr(osvi.dwPlatformId));                             //
end;                                                                //
//////////////////////////////////////////////////////////////////////
